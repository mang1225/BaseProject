...
public class ImageLoader {
  ...
  public static void loadImageWithPlaceHolder(Context context, ImageView imageView, String imageUrl) {
    Glide.with(context).load(imageUrl).placeholder(R.drawable.ic_default).error(R.drawable.ic_default).dontAnimate().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
  }
  ...
  public static void loadCircleImage(Context context, ImageView imageView, String imageUrl) {
    Glide.with(context).load(imageUrl).diskCacheStrategy(DiskCacheStrategy.SOURCE).dontAnimate().transform(new CircleImageTransform(getGlobalContext())).into(imageView);
  }
  ...
  public static void loadImageWithSize(Context context, ImageView imageView, String imageUrl, int width, int height) {
    Glide.with(context).load(imageUrl).diskCacheStrategy(DiskCacheStrategy.SOURCE).override(width, height).into(imageView);
  }
}
