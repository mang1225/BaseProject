...
public class HttpRetrofit {
  ...
  private final static Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").serializeNulls().create();
  ...
  HttpRetrofit() {
    ...
    Retrofit retrofit =
        retrofitBuilder.baseUrl(HttpApi.BASE_URL).client(client).addCallAdapterFactory(RxJavaCallAdapterFactory.create()).addConverterFactory(GsonConverterFactory.create(gson)).build();
